...
from typing import Any, Callable, Literal, TypedDict, TypeVar, Type
...
class _BASE:
    ...
    def comfytype(io_type: str, **kwargs):
        ...
        '''
        def decorator(cls: _BASE.T) -> _BASE.T:
            if isinstance(cls, _BASE._ComfyType) or issubclass(cls, _BASE._ComfyType):
                ...
                # new class
                new_cls: _BASE2.ComfyTypeIO = type(
                    cls.__name__,
                    (cls, _BASE2.ComfyTypeIO),
                    cls_dict
                )
            ...
            new_cls.io_type = _BASE._StringIOType(io_type)
    ...
    class InputV3(_IO_V3):
        ...
        def get_io_type(self):
            return _BASE._StringIOType(self.io_type)
    ...
    class DynamicInput(InputV3, ABC):
        ...
        '''
        @abstractmethod
        def get_dynamic(self) -> list[_BASE.InputV3]:
            ...
    ...
    class DynamicOutput(OutputV3, ABC):
        ...
        @abstractmethod
        def get_dynamic(self) -> list[_BASE.OutputV3]:
            ...


class _BASE2:
    class ComfyTypeI(_BASE._ComfyType):
        '''ComfyType subclass that only has a default Input class - intended for types that only have Inputs.'''
        class Input(_BASE.InputV3):
            ...
    ...
    class ComfyTypeIO(ComfyTypeI):
        ...
        class Output(_BASE.OutputV3):
            ...

    @_BASE.comfytype(io_type="COMBO")
    class Combo(ComfyTypeI):
        Type = str
        class Input(_BASE.WidgetInputV3):
            ...
            def __init__(self, id: str, options: list[str]=None, display_name: str=None, optional=False, tooltip: str=None, lazy: bool=None,
                        ...
                        upload: IO.UploadType=None, image_folder: IO.FolderType=None,
...
class IO:
    ...
    _ComfyType = _BASE._ComfyType
    comfytype = _BASE.comfytype
    ComfyTypeI = _BASE2.ComfyTypeI
    ComfyTypeIO = _BASE2.ComfyTypeIO
    InputV3 = _BASE.InputV3
    OutputV3 = _BASE.OutputV3
    WidgetInputV3 = _BASE.WidgetInputV3
    ...
    @staticmethod
    def Custom(io_type: str) -> type[IO.ComfyTypeIO]:
        '''Create a ComfyType for a custom io_type.'''
        @IO.comfytype(io_type=io_type)
        class CustomComfyType(IO.ComfyTypeIO):
            ...
    ...
    class Int(ComfyTypeIO):
        ...
        class Input(_BASE.WidgetInputV3):
            ...
            def __init__(self, id: str, display_name: str=None, optional=False, tooltip: str=None, lazy: bool=None,
                        ...
                        display_mode: IO.NumberDisplay=None, socketless: bool=None, force_input: bool=None):
                super().__init__(id, display_name, optional, tooltip, lazy, default, socketless, None, force_input)
                self.min = min
                self.max = max
                self.step = step
    ...
    class MultiCombo(ComfyTypeI):
        ...
        Type = list[str]
        class Input(_BASE2.Combo.Input):
    ...
    class MultiType:
        ...
        class Input(_BASE.InputV3):
            ...
            '''
            def __init__(self, id: str | IO.InputV3, types: list[type[IO._ComfyType] | IO._ComfyType], display_name: str=None, optional=False, tooltip: str=None, lazy: bool=None, extra_dict=None):
                # if id is an Input, then use that Input with overridden values
                self.input_override = None
                if isinstance(id, IO.InputV3):
                    ...
                    if isinstance(self.input_override, IO.WidgetInputV3):
                        self.input_override.widget_type = self.input_override.get_io_type()
                super().__init__(id, display_name, optional, tooltip, lazy, extra_dict)
                self._io_types = types

            @property
            def io_types(self) -> list[type[IO.InputV3]]:
                '''
    ...
    class AutogrowDynamic(ComfyTypeI):
        Type = list[Any]
        class Input(_BASE.DynamicInput):
            def __init__(self, id: str, template_input: IO.InputV3, min: int=1, max: int=None,
            ...
            def get_dynamic(self) -> list[IO.InputV3]:
                curr_count = 1
                new_inputs = []
                for i in range(self.min):
                    ...
                    if isinstance(self.template_input, IO.WidgetInputV3):
    ...
    class MatchType(ComfyTypeIO):
        class Template:
            def __init__(self, template_id: str, allowed_types: IO._ComfyType | list[IO._ComfyType]):
                self.template_id = template_id
                self.allowed_types = [allowed_types] if isinstance(allowed_types, IO._ComfyType) else allowed_types
        ...
        class Input(_BASE.DynamicInput):
            def __init__(self, id: str, template: IO.MatchType.Template,
                        display_name: str=None, optional=False, tooltip: str=None, lazy: bool=None, extra_dict=None):
                super().__init__(id, display_name, optional, tooltip, lazy, extra_dict)
        ...
        class Output(_BASE.DynamicOutput):
            def __init__(self, id: str, template: IO.MatchType.Template, display_name: str=None, tooltip: str=None,
                        is_output_list=False):
                super().__init__(id, display_name, tooltip, is_output_list)
                self.template = template

            def get_dynamic(self) -> list[IO.OutputV3]:
                return [self]
    ...
    class HiddenHolder:
        ...
        def from_dict(cls, d: dict | None):
            if d is None:
                d = {}
            return cls(
                unique_id=d.get(IO.Hidden.unique_id, None),
                prompt=d.get(IO.Hidden.prompt, None),
                extra_pnginfo=d.get(IO.Hidden.extra_pnginfo, None),
                dynprompt=d.get(IO.Hidden.dynprompt, None),
                auth_token_comfy_org=d.get(IO.Hidden.auth_token_comfy_org, None),
                api_key_comfy_org=d.get(IO.Hidden.api_key_comfy_org, None),
            )
    ...
    class Schema:
        ...
        inputs: list[IO.InputV3]=None
        outputs: list[IO.OutputV3]=None
        hidden: list[IO.Hidden]=None
        description: str=""
        ...
        def finalize(self):
            ...
            if self.is_api_node:
                if self.hidden is None:
                    self.hidden = []
                if IO.Hidden.auth_token_comfy_org not in self.hidden:
                    self.hidden.append(IO.Hidden.auth_token_comfy_org)
                if IO.Hidden.api_key_comfy_org not in self.hidden:
                    self.hidden.append(IO.Hidden.api_key_comfy_org)
            # if is an output_node, will need prompt and extra_pnginfo
            if self.is_output_node:
                if self.hidden is None:
                    self.hidden = []
                if IO.Hidden.prompt not in self.hidden:
                    self.hidden.append(IO.Hidden.prompt)
                if IO.Hidden.extra_pnginfo not in self.hidden:
                    self.hidden.append(IO.Hidden.extra_pnginfo)
        ...
        def get_v1_info(self, cls) -> IO.NodeInfoV1:
            # get V1 inputs
            input = {
                "required": {}
            }
            if self.inputs:
                for i in self.inputs:
                    if isinstance(i, _BASE.DynamicInput):
                        dynamic_inputs = i.get_dynamic()
                        for d in dynamic_inputs:
                            IO.add_to_dict_v1(d, input)
                    else:
                        IO.add_to_dict_v1(i, input)
            if self.hidden:
        ...
        def get_v3_info(self, cls) -> IO.NodeInfoV3:
            input_dict = {}
            output_dict = {}
            hidden_list = []
            ...
            if self.inputs:
                for input in self.inputs:
                    IO.add_to_dict_v3(input, input_dict)
            if self.outputs:
                for output in self.outputs:
                    IO.add_to_dict_v3(output, output_dict)
            if self.hidden:
                for hidden in self.hidden:
    ...
    @staticmethod
    def add_to_dict_v1(i: IO.InputV3, input: dict):
    ...
    @staticmethod
    def add_to_dict_v3(io: IO.InputV3 | IO.OutputV3, d: dict):
    ...
    class _ComfyNodeBaseInternal(_ComfyNodeInternal):
        ...
        def GET_BASE_CLASS(cls):
            return IO._ComfyNodeBaseInternal
        ...
        @final
        @classmethod
        def EXECUTE_NORMALIZED(cls, *args, **kwargs) -> IO.NodeOutput:
            to_return = cls.execute(*args, **kwargs)
            if to_return is None:
                to_return = IO.NodeOutput()
            elif isinstance(to_return, IO.NodeOutput):
                pass
            elif isinstance(to_return, tuple):
                to_return = IO.NodeOutput(*to_return)
            elif isinstance(to_return, dict):
                to_return = IO.NodeOutput.from_dict(to_return)
            elif isinstance(to_return, ExecutionBlocker):
                to_return = IO.NodeOutput(block_execution=to_return.message)
            else:
        ...
        @final
        @classmethod
        async def EXECUTE_NORMALIZED_ASYNC(cls, *args, **kwargs) -> IO.NodeOutput:
        ...
        @final
        @classmethod
        def PREPARE_CLASS_CLONE(cls, hidden_inputs: dict) -> type[IO.ComfyNode]:
            """Creates clone of real node class to prevent monkey-patching."""
            c_type: type[IO.ComfyNode] = cls if is_class(cls) else type(cls)
            type_clone: type[IO.ComfyNode] = shallow_clone_class(c_type)
            # set hidden
            type_clone.hidden = IO.HiddenHolder.from_dict(hidden_inputs)
            return type_clone
        ...
        @final
        @classmethod
        def INPUT_TYPES(cls, include_hidden=True, return_schema=False) -> dict[str, dict] | tuple[dict[str, dict], IO.Schema]:
    ...
    class NodeOutput:
        ...
        '''
        def __init__(self, *args: Any, ui: IO._UIOutput | dict=None, expand: dict=None, block_execution: str=None):
            self.args = args
            self.ui = ui
            self.expand = expand
        ...
        @classmethod
        def from_dict(cls, data: dict[str, Any]) -> IO.NodeOutput:
            args = ()
            ui = None
            expand = None
            if "result" in data:
...
class UI:
    @staticmethod
    def _get_directory_by_folder_type(folder_type: IO.FolderType) -> str:
        if folder_type == IO.FolderType.input:
    ...
    class SavedResult(dict):
        def __init__(self, filename: str, subfolder: str, type: IO.FolderType):
        ...
        @property
        def type(self) -> IO.FolderType:
            return IO.FolderType(self["type"])


    class SavedImages(IO._UIOutput):
        """A UI output class to represent one or more saved images, potentially animated."""
        def __init__(self, results: list[UI.SavedResult], is_animated: bool = False):
            super().__init__()
    ...
    class SavedAudios(IO._UIOutput):
        """UI wrapper around one or more audio files on disk (FLAC / MP3 / Opus)."""
        def __init__(self, results: list[UI.SavedResult]):
            super().__init__()
            self.results = results
    ...
    class ImageSaveHelper:
        ...
        @staticmethod
        def _create_png_metadata(cls: Type[IO.ComfyNode] | None) -> PngInfo | None:
        ...
        @staticmethod
        def _create_animated_png_metadata(cls: Type[IO.ComfyNode] | None) -> PngInfo | None:
        ...
        @staticmethod
        def _create_webp_metadata(pil_image: PILImage.Image, cls: Type[IO.ComfyNode] | None) -> PILImage.Exif:
        ...
        @staticmethod
        def save_images(
            images, filename_prefix: str, folder_type: IO.FolderType, cls: Type[IO.ComfyNode] | None, compress_level = 4,
        ) -> list[UI.SavedResult]:
            """Saves a batch of images as individual PNG files."""
            full_output_folder, filename, counter, subfolder, _ = folder_paths.get_save_image_path(
                filename_prefix, UI._get_directory_by_folder_type(folder_type), images[0].shape[1], images[0].shape[0]
            )
            results = []
            metadata = UI.ImageSaveHelper._create_png_metadata(cls)
            for batch_number, image_tensor in enumerate(images):
                img = UI.ImageSaveHelper._convert_tensor_to_pil(image_tensor)
                filename_with_batch_num = filename.replace("%batch_num%", str(batch_number))
                file = f"{filename_with_batch_num}_{counter:05}_.png"
                img.save(os.path.join(full_output_folder, file), pnginfo=metadata, compress_level=compress_level)
                results.append(UI.SavedResult(file, subfolder, folder_type))
                counter += 1
            return results

        @staticmethod
        def get_save_images_ui(images, filename_prefix: str, cls: Type[IO.ComfyNode] | None, compress_level=4) -> UI.SavedImages:
            """Saves a batch of images and returns a UI object for the node output."""
            return UI.SavedImages(
        ...
        def save_animated_png(
            images, filename_prefix: str, folder_type: IO.FolderType, cls: Type[IO.ComfyNode] | None, fps: float, compress_level: int
        ) -> UI.SavedResult:
            """Saves a batch of images as a single animated PNG."""
            full_output_folder, filename, counter, subfolder, _ = folder_paths.get_save_image_path(
                filename_prefix, UI._get_directory_by_folder_type(folder_type), images[0].shape[1], images[0].shape[0]
            )
            pil_images = [UI.ImageSaveHelper._convert_tensor_to_pil(img) for img in images]
            metadata = UI.ImageSaveHelper._create_animated_png_metadata(cls)
            ...
            return UI.SavedResult(file, subfolder, folder_type)

        @staticmethod
        def get_save_animated_png_ui(
            images, filename_prefix: str, cls: Type[IO.ComfyNode] | None, fps: float, compress_level: int
        ) -> UI.SavedImages:
            ...
            return UI.SavedImages([result], is_animated=len(images) > 1)

        @staticmethod
        def save_animated_webp(
            images,
            filename_prefix: str,
            folder_type: IO.FolderType,
            ...
            pil_images = [UI.ImageSaveHelper._convert_tensor_to_pil(img) for img in images]
            pil_exif = UI.ImageSaveHelper._create_webp_metadata(pil_images[0], cls)
    ...
    class AudioSaveHelper:
        ...
        def save_audio(
            ...
            full_output_folder, filename, counter, subfolder, _ = folder_paths.get_save_image_path(
                filename_prefix, UI._get_directory_by_folder_type(folder_type)
            )
            ...
            for batch_number, waveform in enumerate(audio["waveform"].cpu()):
                ...
                if format == "opus":
                    if sample_rate > 48000:
                        sample_rate = 48000
                    elif sample_rate not in UI.AudioSaveHelper._OPUS_RATES:
                        # Find the next highest supported rate
                        for rate in sorted(UI.AudioSaveHelper._OPUS_RATES):
                            if rate > sample_rate:
                                sample_rate = rate
                                break
                        if sample_rate not in UI.AudioSaveHelper._OPUS_RATES:  # Fallback if still not supported
                            sample_rate = 48000
        ...
        def get_save_audio_ui(
            audio, filename_prefix: str, cls: Type[IO.ComfyNode] | None, format: str = "flac", quality: str = "128k",
        ) -> UI.SavedAudios:
            """Save and instantly wrap for UI."""
            return UI.SavedAudios(
    ...
    class PreviewImage(IO._UIOutput):
        def __init__(self, image: IO.Image.Type, animated: bool = False, cls: Type[IO.ComfyNode] = None, **kwargs):
            self.values = UI.ImageSaveHelper.save_images(
                image,
    ...
    class PreviewMask(PreviewImage):
        def __init__(self, mask: IO.Mask.Type, animated: bool=False, cls: IO.ComfyNode=None, **kwargs):
            preview = mask.reshape((-1, 1, mask.shape[-2], mask.shape[-1])).movedim(1, -1).expand(-1, -1, -1, 3)
            super().__init__(preview, animated, cls, **kwargs)


    # class UILatent(_UIOutput):
    #     def __init__(self, values: list[UI.SavedResult | dict], **kwargs):
    ...
    class PreviewAudio(IO._UIOutput):
        def __init__(self, audio: dict, cls: Type[IO.ComfyNode] = None, **kwargs):
            self.values = UI.AudioSaveHelper.save_audio(
                audio,
    ...
    class PreviewVideo(IO._UIOutput):
        def __init__(self, values: list[UI.SavedResult | dict], **kwargs):
            self.values = values
    ...
    class PreviewUI3D(IO._UIOutput):
        def __init__(self, values: list[UI.SavedResult | dict], **kwargs):
            self.values = values
    ...
    class PreviewText(IO._UIOutput):
